...
| [workflows/anima_t2i_hires_tagformatter.json](workflows/anima_t2i_hires_tagformatter.json) | 上記に Tag Formatter ノードを組み込んだ版（要 ComfyUI-TagFormatter） |
| [comfyui_nodes/ComfyUI-TagFormatter/](comfyui_nodes/ComfyUI-TagFormatter/) | ComfyUI カスタムノード: タグを 品質→女キャラ→女服→女表情→女ポーズ→画角→男→場所→シチュエーション→時間→天候→光→影→画風→絵師→LoRA→不明 の順に整形、キャラ要素（外見・キャラ名・任意で服）の削除。辞書は txt で編集可 |
