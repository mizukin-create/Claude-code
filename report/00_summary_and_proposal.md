...
| 2 | 本垢は全年齢固定・R-18 はサブ垢へ（一般論） | **本垢の画像投稿が公式 oEmbed API で「not authorized」になり、シャドウバンチェッカーで Search Ban / Search Suggestion Ban が該当**。公開コードでは、アイコン・ヘッダーの画像判定（`NsfwAvatarImage` / `NsfwBannerImage`）と「1 日 3 件以上の投稿画像が成人向け判定」で作者に 7 日期限の NSFW ラベルが付き、検索候補・検索結果・圏外推薦から外れる仕様（06 §3.3〜3.6）。実物のアイコン・ヘッダーは素肌露出の大きい構図 | **最優先**: アイコン・ヘッダーを衣服ありの顔中心に差し替え、2 週間は画像投稿 1 日 2 件以内・露出の少ない題材、自己紹介から R18 / Patreon を外す。「センシティブなメディアとしてマーク」は ON にしない（**9/10 に OFF を確認済み**。9/10 に別アカウントからの閲覧で、4 月の投稿は警告なし・9/8 の露出ゼロの投稿は「X によりラベル付け」の警告つきと確認。運営のアカウントフラグは否定、原因は投稿ごとの X 側ラベルで、画像内容よりアカウント側の状態で判定されている。9 月分の Under the Hood（10/10〜）で確定。06 §3.3 結果 1〜3）。**9/10 07:39 JST にアイコン・ヘッダーの差し替えと自己紹介の修正を実施済み**（06 §2.1・§3.6 実施状況）。R-18 告知投稿 3 件も同日削除済み。残りは 1 日 2 件以内の運用と週次確認 |
...
未確認のまま:
- 本垢に付いている X の可視性ラベル。自己申告のセンシティブ設定は 9/10 に OFF を確認済みなので、残るのは分類器の投稿ラベル（NSFW_HIGH_RECALL 等）か運営の NsfwAdmin。8 月分の Under the Hood は適格投稿 10 件未満で生成されないため、サブ垢からの警告表示テストと新規投稿の check_embed で暫定判別し、9 月分（10/10〜）で確定する（06 §3.3）。
