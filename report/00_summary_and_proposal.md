...
| 週 | やること | 完了の目安 |
|---|---|---|
| 第 1 週 | **アイコン・ヘッダーの差し替え（衣服あり・顔中心）、画像投稿 1 日 2 件以内、自己紹介から R18 / Patreon を外す（06 §3.6）**、環境更新（§5.4）、主役キャラ 1〜2 体の設定シートと固定プロンプト（`prompts/character_sheet_template.md`）、プロフィール・固定ポスト・導線の整備 | 固定プロンプトで 20 枚出して顔・衣装がブレない。1 週間後にチェッカーの Search Suggestion Ban が解除、`tools/check_embed.py` で新規画像投稿が OK |
