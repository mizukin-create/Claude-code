...
| 2 | 本垢は全年齢固定・R-18 はサブ垢へ（一般論） | **本垢の画像投稿が公式 oEmbed API で「not authorized」になり、シャドウバンチェッカーで Search Ban / Search Suggestion Ban が該当**。公開コードでは、アイコン・ヘッダーの画像判定（`NsfwAvatarImage` / `NsfwBannerImage`）と「1 日 3 件以上の投稿画像が成人向け判定」で作者に 7 日期限の NSFW ラベルが付き、検索候補・検索結果・圏外推薦から外れる仕様（06 §3.3〜3.6）。実物のアイコン・ヘッダーは素肌露出の大きい構図 | **最優先**: アイコン・ヘッダーを衣服ありの顔中心に差し替え、2 週間は画像投稿 1 日 2 件以内・露出の少ない題材、自己紹介から R18 / Patreon を外す。「センシティブなメディアとしてマーク」は ON にしない（**9/10 に OFF を確認済み**。9/10 の検証で、9/4〜9/8 の投稿は露出ゼロの絵まで全件 `possibly_sensitive: true`・「X によりラベル付け」の警告つき、同じ画像を別アカウントから投稿すると false・埋め込み OK。X 側がアカウント単位で付けた「メディアをセンシティブ扱い」の状態（NsfwAdmin 相当）で自動では切れず、X ヘルプは「アカウント設定を固定された場合に異議を申し立てる方法は現在ない」と明記。できるのは投稿単位の「センシティブラベルの審査を依頼」とサポートフォームのみ。4 週間で外れなければ全年齢の看板を別アカウントへ（06 §3.3 結果 1〜4・§3.7・§3.8）。**9/10 07:39 JST にアイコン・ヘッダーの差し替えと自己紹介の修正を実施済み**（06 §2.1・§3.6 実施状況）。R-18 告知投稿 3 件も同日削除済み。残りは 1 日 2 件以内の運用と週次確認 |
