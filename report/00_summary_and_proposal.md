...
未確認のまま:
- 本垢に付いている X の可視性ラベル。自己申告のセンシティブ設定は 9/10 に OFF を確認済みなので、残るのは分類器の投稿ラベル（NSFW_HIGH_RECALL 等）か運営の NsfwAdmin。Under the Hood（x.com/i/under_the_hood）の 8 月分で本人が確認する（06 §3.3）。
